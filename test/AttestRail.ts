...
async function submitProfile(
  ...
  await contracts.registry
    ...
    .submitProfile(
      encrypted.handles[0], encrypted.handles[1], encrypted.handles[2],
      encrypted.handles[3], encrypted.handles[4], encrypted.inputProof,
      attestation, sig,
    );
}
...
describe("AttestRail — Full Integration", function () {
  ...
  describe("Eligible path", function () {
    it("full flow: submit profile → create check → public decrypt → gated transfer", async function () {
      ...
      await submitProfile(contracts, signers, {
        kyc: true, jurisdiction: true, sanctions: false, riskTier: 1, exposure: 5000,
      });
      ...
      const event = receipt!.logs.find(
        (l: any) => l.fragment?.name === "EligibilityCheckCreated",
      ) as any;
      const checkId = event.args[0];
      ...
      await contracts.token
        .connect(signers.investor)
        .gatedTransfer(signers.recipient.address, 10_000, checkId);
      ...
      const investorClear = await fhevm.userDecryptEuint(
        FhevmType.euint64, investorBal,
        await contracts.token.getAddress(), signers.investor,
      );
      const recipientClear = await fhevm.userDecryptEuint(
        FhevmType.euint64, recipientBal,
        await contracts.token.getAddress(), signers.recipient,
      );
  ...
  describe("Sanctioned blocked", function () {
    it("sanctioned investor → eligible=false → zero-amount transfer", async function () {
      await submitProfile(contracts, signers, {
        kyc: true, jurisdiction: true, sanctions: true, riskTier: 1, exposure: 5000,
      });
      const policyId = await createPolicy(contracts, signers, 100_000, 10_000_000);
      await contracts.token.connect(signers.issuer).mint(signers.investor.address, 50_000);
      ...
      const tx = await contracts.gate
        ...
        .createEligibilityCheck(policyId, signers.recipient.address, 10_000);
      const receipt = await tx.wait();
      const checkId = (receipt!.logs.find((l: any) => l.fragment?.name === "EligibilityCheckCreated") as any).args[0];
  ...
  describe("Per-user exposure cap blocked", function () {
    it("exposure + amount > maxExposure → eligible=false", async function () {
      ...
      await submitProfile(contracts, signers, {
        kyc: true, jurisdiction: true, sanctions: false, riskTier: 1, exposure: 90_000,
      });
      const policyId = await createPolicy(contracts, signers, 100_000, 10_000_000);
  ...
  describe("Aggregate cap blocked", function () {
    it("aggregate + amount > issuerCap → eligible=false", async function () {
      ...
      await submitProfile(contracts, signers, {
        kyc: true, jurisdiction: true, sanctions: false, riskTier: 1, exposure: 0,
      });
      ...
      const checkId1 = (r1!.logs.find((l: any) => l.fragment?.name === "EligibilityCheckCreated") as any).args[0];
      ...
      const checkId2 = (r2!.logs.find((l: any) => l.fragment?.name === "EligibilityCheckCreated") as any).args[0];
